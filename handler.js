...
module.exports = {
  async handler(m) {
    try {
      ...
      let isROwner = [global.conn.user.jid, ...global.owner].map(v => v.replace(5521970112816g, '') + '@s.whatsapp.net').includes(m.sender)
      let isOwner = isROwner || m.fromMe
      let isMods = isOwner || global.mods.map(v => v.replace(5521970112816g, '') + '@s.whatsapp.net').includes(m.sender)
      let isPrems = isROwner || global.prems.map(v => v.replace(5521970112816g, '') + '@s.whatsapp.net').includes(m.sender)
