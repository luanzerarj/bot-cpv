...
async function starts() {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const { text, extendedText, contact, location, liveLocation, image, video, sticker, document, audio, product, atetepe, tetepe, instagram } = MessageType
			...
			switch(command) {
				...
				/*case 'rgbtext':
					...
					break*/
				case 'attp':
						if (args.length < 1) return reply('*Qual o texto, filho da puta?*')
						teks = body.slice(6)
						const atetepe = await getBuffer(`https://api.xteam.xyz/attp?file&text=${teks}`)
						client.sendMessage(from, atetepe, sticker, {quoted: mek})
					break
				case 'ttp':
						if (args.length < 1) return reply('*Qual o texto, filho da puta?*')
						teks = body.slice(5)
						const tetepe = await getBuffer(`https://api.xteam.xyz/ttp?file&text=${teks}`)
						client.sendMessage(from, tetepe, sticker, {quoted: mek})
					break
				case 'insta':
						if (args.length < 1) return reply('*Qual a porra do usuario?*')
						teks = body.slice(10)
						const instagram = await getBuffer(`https://api.xteam.xyz/dl/igstalk?nama=bintang_nur_pradana&APIKEY=${teks}`)
						client.sendMessage(from, instagram, sticker, {quoted: mek})
				case 'geral':
				...
				case 'mamei':
					...
					if (mentioned.length > 1) {
						for (let _ of mentioned) {
							...
							mentions(`*O @${mentioned[0].split('@')[0]} MAMOU GOSTOSO GLUB GLUB GLUB*`, mentioned, true)
							client.fuzil(from, mentioned)
						}
							break
		       	case 'messidatrufa':
		        ...
		        case 'textaogabiru':
				    if (args.length < 1) return reply ('*Textao do Gabiru:*\n*Cara, tu acha que eu sou frustrado? E onde em sua sã consciência tu foi achar q eu invejo você cara? Vc já viu alguém invejar qm n tem nada? Inveja eu tenho do Bill Gates, de você eu tenho pena, o único lugar que você se sente alguém é auqi no grupo pq tem um monte de retardado que bota pilha nas merdas que você fala, na sua casa vive brigando com sua irmã e nem conversa com seus pais, msm assim eles tão aí pagando suas contas e te dando boa vida, certeza que sua família te deserdaria se visse oque você fala na internet, n tenho oq falar mais de você pq eu n vejo nada em você cara, é simplesmente NADA, então para de achar q vai se dar bem fzd textao e espelhando suas frustrações pessoas por favor.*')
		            		break
				case 'textaogabiru2':
				    if (args.length < 1) return reply ('*Textao do Gabiru²:*\n\nEntendivel\nAlienado pra caralho\nPara de ouvir o rap q tu ouve, se n representa o movimento n\nSe ouve rap mas n entende\nMlk retardado\nTira uns print e posta na cpv tbm\nPovo vai dá risada\nAproveita e pede grana pra sua coroa pra compra maconha\nDps fuma no seu quarto no apartamento\nMó boy')
		            		break
		        case 'textaodanilo':
